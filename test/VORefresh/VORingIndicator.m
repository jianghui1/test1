...
- (instancetype)initWithFrame: (CGRect)frame
	...
	if (self) {
		...
		label.font = [UIFont boldSystemFontOfSize:diameter * 0.5];
		label.text = @"趣";
...
- (void)setupAnimationLayer{
    ...
    arcLayer.path                  = [self layoutPathWithScale:0.9].CGPath;
...
+ (NSArray *)gradientColorArrayWithColor: (UIColor *)color{
    ...
    //转动拖尾渐变效果
	return @[(id)[color colorWithAlphaComponent:1.0].CGColor, (id)[color colorWithAlphaComponent:1.0].CGColor, (id)[color colorWithAlphaComponent:1.0].CGColor];
}
