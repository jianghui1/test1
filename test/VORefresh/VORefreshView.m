...
- (void)adjustSubViewFrame{
	...
	if (text) {
		...
		// 设置activityView和progressView
		center = SELF_IS_ERECT ? CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMaxY(self.bounds) / 2) : CGPointMake(CGRectGetMaxX(self.bounds) / 2, CGRectGetMidY(self.bounds));
	}
	else{
